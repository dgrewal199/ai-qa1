...
class QALogicService:
    ...
    async def _run_executor(
        ...
        Streams agent events through ExecutionProgressTracker for
        live per-query progress output in the terminal.
        """
        from report_formatter import ExecutionProgressTracker, extract_tc_descriptions
        ...
        # Build description lookup for progress display
        descriptions = extract_tc_descriptions(sql_json)
        total_cases  = len(descriptions) or 1
        ...
        async for event in self.executor_runner.run_async(
            ...
            if event.content and event.content.parts:
                for part in event.content.parts:
                    if part.text:
                        # Stream to progress tracker immediately
